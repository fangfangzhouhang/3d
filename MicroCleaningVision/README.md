...
- 当前回归：项目`.venv`中81项测试全部执行并通过，无跳过。
- `public_001.jpg` 已完成人工Mask与HSV预测比较：IoU为0.0739，证明链路可运行，也证明当前HSV基线在此图上失败。
- U500 USB相机适配器：已实现并通过FakeVideoCapture测试；尚未运行实机probe，团队采集数据仍未进入证据链。
- 像素到毫米标定：尚未验证。
...
4. [团队入门指南](docs/总流程说明/团队入门指南.md)
5. [本轮任务看板](docs/总流程说明/团队任务看板.md)
6. [成员A工作流程与命令百科](docs/成员A/成员A_工作流程与命令百科.md)
7. [U500 USB数码显微镜接入指南](docs/成员A/U500_USB数码显微镜接入指南.md)
8. [硬件组说明与接口](docs/硬件组/README.md)
...
详细入口、出口、三人任务和停止条件见 [长期分阶段研发规划](docs/未来计划/长期分阶段研发规划.md)。
