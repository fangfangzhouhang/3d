...
> **2026-09-07 头注：** 上位机接口只有本文开头的 **MCV1** 表（`MCV1|PING` 等）。下文「串口协议」一节仍列出旧命令 `PING` / `ARM` / `PUMP n`，**不是** 电脑端当前接口；发给板子会被当成 `BAD_FORMAT`。烧录与验收以 [项目作战总表](../../../说明文档/总流程说明/项目作战总表.md) 的 MVP-1 为准。本轮不重写全文。
